...
export async function runAgentSession({
  ...
  const agentAddendumPlan = [
    ...
    '   - Example: "React" → assume latest React (e.g., React 18 or 19), NOT old versions',
    '',
    ...
    '   - If user explicitly mentions a version (e.g., "Next.js 14"), honor that specific version',
    ...
    '   - Example: "Next.js: Latest stable is 15.x (released 2024), previous major: 14.x"',
    '',
    'Example terms requiring clarification:',
    '- Technology names: "Next.js" → verify current version (assume latest unless specified)',
    '- API names: "OpenAI API" → verify current API version/endpoint names (latest)',
    '- Product names: "ChatGPT" → verify current model names/versions (newest available)',
    '- Ambiguous terms: "新しいバージョン" → search to find what "new" actually means (latest stable)',
    '',
  ...
  const agentAddendumResearch = [
    ...
    '',
    '4. VERSION-SPECIFIC RESEARCH (CRITICAL):',
