...
export const flowInstruction = [
  ...
  '- CLARIFY: verify names/versions only, internal note (fact check + URL hunt).',
  '- PLAN: list unknowns and craft research queries, internal note (fact check + URL hunt).',
  '- RESEARCH: execute plan with tools and log facts, internal note (fact check + URL hunt).',
  '- CONTROL: judge coverage/quality and pick research or final, internal note (fact check + URL hunt).',
  '- FINAL: write the user-facing answer using the internal memos plus direct urlContext (browse) fetches of FINAL_URLS; ground every claim in fetched content.',
  'CLARIFY through CONTROL are strictly for factual verification and collecting URLs—no user-facing synthesis. FINAL turns those notes + fetched URLs into the detailed response.',
...
export const agentAddendumPlan = [
  ...
  '- Do NOT doubt or override CLARIFY unless you verify by running your own googleSearch (concise_search)/urlContext (browse) now in PLAN.',
...
export const agentAddendumResearch = [
  ...
  '',
  'Must do:',
  '- Call googleSearch (concise_search) and/or urlContext (browse) (at least one tool call).',
  ...
  '- Populate FINAL_URLS with title + URL + authority (prefer official/high-trust) so CONTROL/FINAL can urlContext (browse) them.',
...
export const agentAddendumControl = [
  ...
  '',
  'Must do (text-only; no tools in CONTROL):',
  '- Review UNKNOWN_FACTS vs FACTS_EXTRACTED.',
  '- Weigh source quality (official, multiple, fresh).',
  '- Tag discipline: after "Thinking Process:<AGENT>" open a single <CONTROL_DECISION> block immediately; close it as the final characters. No extra text before or after.',
  '- Output a single <CONTROL_DECISION>...</CONTROL_DECISION> block as plain text with ONLY these sections (no extra prose, no text outside tags).',
  '  CONTROL_DECISION:',
  '  SCORING_CALCULATION:',
  '  - Coverage:',
  '  - Quality:',
  ...
  '  CONTROL_SUMMARY:',
  '  - FACTS_READY:',
  '  - FINAL_URLS_READY: (mandatory; list at least one URL or "None")',
  '    - [title] ([URL]) - [authority: official/reputable/community]',
  '    - [add more URLs as needed]',
  '  - ANSWER_PLAN: (outline for FINAL that maps collected facts/URLs to answer sections; include "Do NOT" notes for incorrect/unsafe answer patterns to avoid)',
  '',
  '  NEXT_RESEARCH_TARGETS:',
  '  - [If action=research: list targets]',
  '  - [If action=final: write "None"]',
  '',
  '- If subqueries from PLAN remain unanswered, prefer action=research to finish Stage 1 before main queries.',
  '- Update PLAN elements (ANSWER_GUIDE/UNKNOWN_FACTS/RESEARCH_PLAN) as needed to close gaps (record updates in PLAN_UPDATES).',
  '',
...
export const criticalAgentRules = [
  ...
  'Internal vs. user-facing: CLARIFY/PLAN/RESEARCH/CONTROL are internal only; FINAL is the only user-facing step.',
  'Internal steps must NOT provide user-facing answers; keep all content inside the required tags for that step.',
  'Tag discipline: after "Thinking Process:<AGENT>" you must open the required tag immediately and close it as the final characters; no text or whitespace outside the tags. Violating this invalidates the output.',
  'FINAL never uses tags or the AGENT marker.',
  ...
  'URL handling: never invent URLs; obey URL OUTPUT POLICY; when URLs are provided, ground facts in fetched content rather than memory.',
...
export const clarifyTurnPrompt = [
  ...
  'Flag any deprecated/obsolete terms, versions, or APIs.',
  'Call googleSearch (concise_search) at least once.',
...
export const planTurnPrompt = [
  ...
  'Trust CLARIFY_OUTPUT (real searches, correct as of that step). If doubting, verify now with googleSearch (concise_search)/urlContext (browse).',
  ...
  'Flag any deprecated/obsolete guidance; prefer current versions and note freshness requirements.',
...
export const researchTurnPrompt = [
  'Thinking Process:<AGENT>',
  'You are thinking as <AGENT>. Do NOT answer the user.',
  '<RESEARCH_NOTES>...</RESEARCH_NOTES> only. Opening tag must follow immediately; closing tag must be the final characters. No text or whitespace outside.',
  'Format discipline:',
  '- Allowed tags: <RESEARCH_NOTES> only (plus Thinking Process trigger line).',
  '- Inside RESEARCH_NOTES, use the section headers and bullet items as plain text; do not add inner tags.',
  ...
  'Flag deprecated/obsolete info; check dates/versions and prefer current official sources.',
  ...
  'FINAL_URLS exist to give CONTROL/FINAL concrete official docs to fetch via urlContext (browse) before answering. Include every URL needed for the final answer.',
...
export const controlTurnPrompt = [
  'Thinking Process:<AGENT>',
  'You are thinking as <AGENT>. Do NOT answer the user.',
  '<CONTROL_DECISION>...</CONTROL_DECISION> only. Opening tag must follow immediately; closing tag must be the final characters. No text or whitespace outside.',
  ...
  'Format discipline:',
  '- Allowed tags: <CONTROL_DECISION> only (plus Thinking Process trigger line).',
  '- Inside CONTROL_DECISION, sections are plain text headings and bullet items; no inner tags. Include a concise ANSWER_PLAN (outline for FINAL, with "Do NOT" items to avoid incorrect/unsafe responses).',
  ...
  'Include sections (no inner tags): SCORING_CALCULATION, OVERRIDE_CHECK, COVERAGE_ANALYSIS, QUALITY_ANALYSIS, CONFIDENCE_ANALYSIS, DECISION, CONTROL_SUMMARY (with ANSWER_PLAN+Do NOT items), PLAN_UPDATES, NEXT_RESEARCH_TARGETS.',
  'CONTROL_SUMMARY must be self-contained; FINAL will rely ONLY on CONTROL_SUMMARY + FINAL_URLS_READY + urlContext results.',
  'FINAL_URLS_READY is mandatory: include at least one URL (or "None" if truly none). Prefer official/authority URLs and format as "- [title] ([URL]) - [authority]".',
  'Tools are NOT available in CONTROL; write the <CONTROL_DECISION> block directly with the required sections and DECISION set to "research" or "final".',
  ...
  'If any fact/feature/API is deprecated/legacy, label it explicitly and steer FINAL toward current guidance.',
].join('\n')

export const finalTurnPrompt = [
  'Use urlContext (browse) when FINAL_URLS exist; if FINAL_URLS is "None", proceed without tool calls unless explicitly instructed otherwise.',
  'When FINAL_URLS are provided, treat them as official/primary evidence: fetch every URL and re-verify facts against the fetched content before answering. Prioritize URLs marked as official/authority.',
  'If FINAL_URLS is not "None", you MUST call urlContext (browse) for every URL before writing any user-facing text. Do NOT second-guess URL formats; attempt every provided URL (these may be Google redirect URLs for AI use).',
  ...
  'Ground every claim in CONTROL_SUMMARY and retrieved document content (urlContext (browse)); do not lean on training data when URLs were supplied.',
...
export const finalSystemInstruction = [
  ...
  'Use VERIFIED_TERMS to keep terminology/versioning consistent, and follow ANSWER_PLAN (including "Do NOT" constraints) to structure the response; treat ANSWER_PLAN as the outline.',
